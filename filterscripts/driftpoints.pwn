...
/*        new Float:h;
        ...
        ////////////////////////////////////////////////////
        CallRemoteFunction("AtualizarPontosDrift", "dd", playerid,DriftPointsNow[playerid]);
...
public Drift(){ // By Luby
        ...
        for(new g=0;g<=GetMaxPlayers();g++){
			    ...
			    if(CallRemoteFunction("ModoDriftAtivo", "d", g) == 0) continue;
...
{
	if(DriftMode[playerid] == false || DriftPointsNow[playerid] < 100) return 1;
	KillTimer(DriftTimer[playerid]);
   	DriftExit(playerid);
   	GameTextForPlayer(playerid,"_~n~_~n~_~n~_~n~_~n~_~n~_~n~_~r~Perdeu!",800,5);
...
public CheckPlayerState(){ //by Abhinav
	...
	for(i=0;i<=GetMaxPlayers();i++){
	    ...
	    if(CallRemoteFunction("ModoDriftAtivo", "d", i) == 0) continue;
	    cs=GetPlayerState(i);
...
//by NicK
forward PontosFeitos(playerid);
public PontosFeitos(playerid) return DriftPointsNow[playerid];
forward BonusFeitos(playerid);
public BonusFeitos(playerid) return DriftBonus[playerid];
forward FazendoDrift(playerid);
public FazendoDrift(playerid) return (DriftPointsNow[playerid] > 70) ? (1) : (0);
forward Drift_SetPos(playerid, Float:x, Float:y, Float:z);
public Drift_SetPos(playerid, Float:x, Float:y, Float:z) {
	SavedPos[playerid][ sX ] = x;
    SavedPos[playerid][ sY ] = y;
    SavedPos[playerid][ sZ ] = z;
    ppos[playerid][0] = x;
    ppos[playerid][1] = y;
    ppos[playerid][2] = z;
    return 1;
